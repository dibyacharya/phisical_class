import { useState, useEffect, useRef } from "react";
import { LiveKitRoom, RoomAudioRenderer, useTracks } from "@livekit/components-react";
import { Track } from "livekit-client";
import "@livekit/components-styles";
import { X, Radio, Volume2, VolumeX, AlertCircle, Loader2 } from "lucide-react";
import api from "../services/api";

/**
 * Live-watch modal for in-progress Windows recordings (v2.1.0).
 *
 * The Windows pipeline publishes its composite (screen + camera PIP at 720p)
 * via a LiveKit RTMP Ingress that the backend allocated when the class started.
 * Unlike the Android side (which uses LiveKit native publisher with two
 * separate tracks), the Windows ingest already arrives as ONE track —
 * the device-side ffmpeg did the composite. So this modal only ever shows
 * a single video tile + audio.
 *
 * Endpoint: GET /api/windows/live-watch/viewer-token?deviceId=&recordingId=
 */
export default function WindowsLiveWatchModal({
  deviceId,
  title,
  onClose,
}) {
  const [creds, setCreds] = useState(null); // { wsUrl, token, roomName }
  const [error, setError] = useState(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    if (!deviceId) {
      setError("deviceId is required");
      setLoading(false);
      return;
    }
    let cancelled = false;
    setLoading(true);
    setError(null);
    // deviceId-only path: backend resolves the active ingress for this
    // device. Avoids the device-heartbeat exposing internal recordingId.
    api
      .get("/windows/live-watch/viewer-token", {
        params: { deviceId },
      })
      .then((res) => {
        if (cancelled) return;
        if (!res.data?.token || !res.data?.wsUrl) {
          setError("Backend returned no LiveKit credentials");
        } else {
          setCreds(res.data);
        }
      })
      .catch((err) => {
        if (cancelled) return;
        const msg =
          err.response?.data?.error ||
          err.message ||
          "Failed to get watch token";
        setError(msg);
      })
      .finally(() => {
        if (!cancelled) setLoading(false);
      });
    return () => {
      cancelled = true;
    };
  }, [deviceId]);

  return (
    <div
      className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 backdrop-blur-sm"
      onClick={(e) => {
        if (e.target === e.currentTarget) onClose();
      }}
    >
      <div className="relative w-full max-w-5xl mx-4 bg-gray-900 rounded-2xl overflow-hidden shadow-2xl">
        <div className="flex items-center justify-between px-5 py-3 bg-gray-950 border-b border-gray-800">
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2 px-2 py-1 rounded-md bg-red-900/40 border border-red-700/50">
              <Radio size={14} className="text-red-400 animate-pulse" />
              <span className="text-xs font-semibold text-red-300 tracking-wide">
                LIVE
              </span>
            </div>
            <h2 className="text-white font-medium text-sm truncate max-w-md">
              {title || "Windows class"}
            </h2>
          </div>
          <button
            onClick={onClose}
            className="p-1.5 rounded-lg hover:bg-gray-800 text-gray-400 hover:text-white transition"
            aria-label="Close"
          >
            <X size={18} />
          </button>
        </div>

        <div className="aspect-video bg-black relative">
          {loading && (
            <div className="absolute inset-0 flex flex-col items-center justify-center text-gray-300">
              <Loader2 size={32} className="animate-spin mb-3" />
              <div className="text-sm">Connecting to live class…</div>
            </div>
          )}

          {error && !loading && (
            <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-6">
              <AlertCircle size={32} className="text-red-400 mb-3" />
              <div className="text-sm text-gray-200 mb-1">Cannot watch live</div>
              <div className="text-xs text-gray-400 max-w-md">{error}</div>
              <button
                onClick={onClose}
                className="mt-4 px-4 py-2 text-xs rounded-lg bg-gray-800 text-gray-200 hover:bg-gray-700"
              >
                Close
              </button>
            </div>
          )}

          {creds && !error && (
            <LiveKitRoom
              serverUrl={creds.wsUrl}
              token={creds.token}
              connect={true}
              audio={false}
              video={false}
              data-lk-theme="default"
              className="h-full"
              onError={(e) => setError(e?.message || "LiveKit connection error")}
            >
              <SingleCompositeViewer />
              <RoomAudioRenderer />
            </LiveKitRoom>
          )}
        </div>

        <div className="px-5 py-2 bg-gray-950 text-[11px] text-gray-500 border-t border-gray-800">
          Live stream from the classroom Windows PC — composite (screen +
          teacher camera PIP) at 720p. Audio is the classroom mic.
        </div>
      </div>
    </div>
  );
}

/** Windows pipeline always produces ONE composited video track. */
function SingleCompositeViewer() {
  const tracks = useTracks(
    [Track.Source.Camera, Track.Source.ScreenShare, Track.Source.Microphone],
    { onlySubscribed: true }
  );
  const video = tracks.find((t) => t.publication?.kind === "video");
  const micPublished = tracks.some(
    (t) => t.publication?.source === Track.Source.Microphone
  );

  if (!video) {
    return (
      <div className="absolute inset-0 flex flex-col items-center justify-center text-gray-400">
        <Loader2 size={28} className="animate-spin mb-3" />
        <div className="text-sm">Waiting for the device to start publishing…</div>
        <div className="text-[11px] mt-1 text-gray-500">
          (Connected to room — no tracks yet)
        </div>
      </div>
    );
  }

  return (
    <div className="absolute inset-0 bg-black">
      <VideoTile track={video} />
      <div className="absolute bottom-3 left-3 flex items-center gap-2 px-2 py-1 rounded-md bg-black/60 text-[11px] text-gray-200">
        {micPublished ? (
          <>
            <Volume2 size={12} className="text-green-400" />
            <span>Mic on</span>
          </>
        ) : (
          <>
            <VolumeX size={12} className="text-gray-400" />
            <span>No mic</span>
          </>
        )}
      </div>
    </div>
  );
}

function VideoTile({ track }) {
  const videoRef = useRef(null);
  useEffect(() => {
    const el = videoRef.current;
    const mediaTrack = track?.publication?.track;
    if (!el || !mediaTrack) return;
    mediaTrack.attach(el);
    el.play().catch((err) => {
      console.warn("[WindowsLiveWatch] video.play() rejected:", err?.message || err);
    });
    return () => {
      try {
        mediaTrack.detach(el);
      } catch (_) {}
    };
  }, [track]);

  return (
    <video
      ref={videoRef}
      autoPlay
      playsInline
      muted
      className="w-full h-full object-contain bg-black"
    />
  );
}
